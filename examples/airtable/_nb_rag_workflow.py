"""Generate the Airtable RAG workflow notebook.

Creates examples/airtable/rag_workflow.ipynb — a notebook that downloads
file attachments from Airtable, indexes them with ffai RAG, and runs
workflow steps as RAG queries.

Usage:
    python examples/airtable/_nb_rag_workflow.py
...
# Airtable RAG Workflow Tutorial — Document Q&A

This notebook demonstrates how to combine **ffai RAG** (Retrieval-Augmented Generation)
with the **Airtable adapter** to:
...
3. Index documents into a ChromaDB vector store with local sentence-transformer embeddings
...
```bash
pip install ffai-workflow-adapters[airtable] unstructured chromadb sentence-transformers httpx
```
...
## Section 4: Initialize RAG Pipeline

Create a RAG instance with:
- **Embeddings**: `all-MiniLM-L6-v2` (local, via sentence-transformers — no API key needed)
- **Vector store**: ChromaDB (persisted to `ffai_data/chroma/`)
- **Chunker**: Recursive (splits on paragraphs, then sentences, then characters)
...
print(f"  Store:      ChromaDB ({CHROMA_DIR})")
...
Chunk each document's text, compute embeddings, and store in ChromaDB.
...
## Section 8: Execute Workflow with RAG (Per-Document)
...
instead of a mashup across all 7 documents.
...
Each record is tagged with its source document name. Passthrough columns and extra columns
(run_id, run_date) are included.
""")
...
This notebook demonstrated the full RAG + Airtable integration:
...
3. **Indexed** documents into ChromaDB with local `all-MiniLM-L6-v2` embeddings
...
Each document is processed independently — the workflow runs all 5 steps for each
document, using source filtering to restrict retrieval to that document's chunks.
Template variables (`{{summarize.response}}`) chain answers between steps within
the same document.
""")
...
output_path = "examples/airtable/rag_workflow.ipynb"
